...
describe("Home Page tests", () => {
  ...
  test("Ver se o botao de adicionar produto funciona" , () => {
    const { getByText } = render(<App />);
    const botaoAdd = getByText(/Adicionar/i);
    userEvent.click(botaoAdd);
    expect(contador).toHaveTextContent("item: + 1");
  })

  test("Indicador de quantidade de produtos começa em 2 , tem que ser falso", () => {
    const { getByText } = render(<App />);
    const indicador = getByText(/Quantidade de Produto/i); // case insensitive
    expect(indicador.textContent).toBe("Quantidade de produtos: 2");
    expect(indicador).toHaveTextContent("Quantidade de produtos: 2"); // ou assim
  });
});
